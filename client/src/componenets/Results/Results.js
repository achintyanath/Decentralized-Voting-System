...
export default class Result extends Component {
    ...
    componentDidMount = async () => {
        ...
        try {
            // Trying for network provider and web3 instance.
            const web3 = await getWeb3();

            // Using web3 to get the user accounts
            const accountsVar = await web3.eth.getAccounts();

            // Geting contract instance.
            const networkIdVar = await web3.eth.net.getId();
            const deployedNetwork = Election.networks[networkIdVar];
            const inst = new web3.eth.Contract(
                Election.abi,
                deployedNetwork && deployedNetwork.address
            );

            // Setting web3, accounts, and contract to the state,
            this.setState({ web3, ElectionInstance: inst, account: accountsVar[0] });
            ...
            const startVar = await this.state.ElectionInstance.methods.getStart().call();
            this.setState({ isElStarted: startVar });
            const endVar = await this.state.ElectionInstance.methods.getEnd().call();
            this.setState({ isElEnded: endVar });
            ...
            const adminVar = await this.state.ElectionInstance.methods.getAdmin().call();
            if (this.state.account === adminVar) {
